...
#create timeseries for leftover data x needs (samples,num_steps,features28) and y needs actions
def leftover_timeseries(indices,testX,truth):
    dataX = []
    dataY = []
    trueList = []
    for i in range(len(indices)):
        dataX.append(testX[indices[i],:,:28])
        dataY.append(testX[indices[i],:,28:])
...
#reading data
input = np.load("../Datasets/datasets_simp_whiterandom/Simpadv_50attack_test.npy", allow_pickle=True)
...
np.save('fgsm_data_simp/query_simpX.npy', testX)


binary_model = load_model('../BinaryNets/LSTMSimpWhiteRandom.keras')
...
model0 = load_model('../SeparateAgents/Agent0NetworkSimp.keras')
model1 = load_model('../SeparateAgents/Agent1NetworkSimp.keras')
model2 = load_model('../SeparateAgents/Agent2NetworkSimp.keras')
...
np.save('fgsm_data_simp/query_simpY.npy', finalQueryY)
