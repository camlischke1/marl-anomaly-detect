...
X = np.load('fgsm_data_simp/query_simpX.npy')
Y = np.load('fgsm_data_simp/query_simpY.npy')
testX = X[200000:]
testY = Y[200000:]
...
for j in range(11,21):
    ...
    np.save("fgsm_test_simpX" + str(np.round(epsilon,2) * 100),attacked_X)
    print(trueActions.shape)
    print(np.unique(trueActions))
    np.save("fgsm_test_simpY"+ str(np.round(epsilon,2) * 100),newY)
    np.save("fgsm_simp_trueActions"+ str(np.round(epsilon,2) * 100),trueActions)