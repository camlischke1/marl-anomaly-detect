...
def attack(testX,testY,epsilon):
    model = load_model('ReplicaSimpEnsembleLogits.keras')
...
#reading data
input = np.load("../Datasets/datasets_simp_blacktimed/phys_decept_blackbox_timed_25.npy", allow_pickle=True)
...
testX = attack(testX,testY,0.9)
print(testX.shape)
print(testY.shape)
print(np.unique(testY,return_counts=True))


binary_model = load_model('../BinaryNets/LSTMSimpBlackTimed.keras')
