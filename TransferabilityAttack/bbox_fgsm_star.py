...
model = load_model('ReplicaStarEnsembleLogits.keras')
...
for j in range(1,21):
    ...
    for i in range(num_iter):
        if testY[i] == 1:
            ...
            adv_example_untargeted = np.concatenate((adv_example_untargeted[:,:,:30],vector[:,:,30:]),axis=2)
    ...
    np.save("fgsm_test_starX" + str(np.round(epsilon,2) * 100),attacked_X)
    print(trueActions.shape)
    print(np.unique(trueActions))
    np.save("fgsm_test_starY"+ str(np.round(epsilon,2) * 100),newY)
    np.save("fgsm_star_trueActions"+ str(np.round(epsilon,2) * 100),trueActions)